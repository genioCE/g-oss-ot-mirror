# OT mirror (read‑only) — OPC UA → MQTT/Kafka → TSDB/Lake
...
**Integrates with:** g-oss-tsdb, g-oss-core, g-oss-stream, g-oss-observability.
    ...
    - Helm chart skeleton is under `charts/g-oss-ot-mirror` (namespace defaults to `platform`).
    ...
    - For Helm, set values in `charts/g-oss-ot-mirror/values.yaml`.
